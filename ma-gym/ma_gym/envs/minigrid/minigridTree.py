...
class MinigridTree(gym.Env):
    ...
    def __update_agent_pos(self, agent_i, move):
        ...
        moves = {
            1: [curr_pos[0]-1, curr_pos[1]],   #Move up
            2: [curr_pos[0], curr_pos[1] - 1], # move left
            3: [curr_pos[0], curr_pos[1] + 1], # move right
            4: [curr_pos[0] + 1, curr_pos[1]], # move down
            5: [curr_pos[0], curr_pos[1]]      #break
        }
    ...
    def _check_if_item_breakable(self):
            ...
            which_tree = None
            for agent_i in range(self.n_agents):
                next_tree, which_tree_new =  self._is_agent_next_to_tree(self.agent_pos[agent_i])
                if next_tree and len(which_tree_new)>0:
                    if len(which_tree_old) == 0:
                        which_tree_old = which_tree_new
                    else:
                        for which_tree_i in which_tree_old:
                            if which_tree_i in which_tree_new:
                                next_to_tree_flag = True
                                which_tree = which_tree_i
                                break
                else:
                    next_to_tree_flag = False
                    return False, None
            return next_to_tree_flag, which_tree
